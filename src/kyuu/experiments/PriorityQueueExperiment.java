...
import kyuu.fast.FastPriorityQueue;
...
public class PriorityQueueExperiment extends Task {
    ...
    public void run() {
        if (uc.getEnergyLeft() > 10000) {
            ...
            c.loggerAlways.log("kyuu faster spent: %d", energy_before_pqff - uc.getEnergyLeft());
            ...
            int energy_before_pqff_g = uc.getEnergyLeft();
            kyuu.fast.FastPriorityQueue<Location> pq_ff_g = new kyuu.fast.FastPriorityQueue<>(100);
            for (int i = 0; i < 50; i++) {
                pq_ff_g.offer(new Location(nextRandom(uc), nextRandom(uc)), nextRandom(uc));
            }
            c.loggerAlways.log("kyuu faster g spent: %d", energy_before_pqff_g - uc.getEnergyLeft());

            while (!pq_ff_g.isEmpty()) {
                pq_ff_g.poll();
            }
