...
class Match:
    def __init__(self, repr_str: str = None, map_name: str = None, player_1: str = None, player_2: str = None,
        ...
        if repr_str is None:
            ...
            self.seed: int = seed
            self.repr: str = '{},{},{},{},{}'.format(map_name, player_1, player_2, match_id, seed)
        else:
            if repr_str.count(',') != 4:
                raise ValueError("Expected repr format: map,player_1,player_2,id,seed -- got: " + repr_str)
            self.repr = repr_str
            self.map, self.player_1, self.player_2, self.id, self.seed = self.repr.split(',')
    ...
    def __init__(self, repr_str: str = None, match: Match = None, winner: str = None, winner_side: str = None,
        ...
        if repr_str is None:
            ...
            self.repr: str = '{},{},{},{},{},{},{},{},{}'.format(
                match.map, match.player_1, match.player_2, match.id, match.seed, winner, winner_side, round_count, win_reason)
        else:
            if repr_str.count(',') != 8:
                raise ValueError("Expected repr format: map,player_1,player_2,id,seed,winner,winner_side,round_count,"
                                 "win_reason -- got: "  + repr_str)
            self.repr = repr_str
            self.match = Match(repr_str=','.join(self.repr.split(',')[:5]))
            self.winner, self.winner_side, self.round_count, self.win_reason = self.repr.split(',')[5:]
...
class MatchResultList:
    ...
    def append(self, result: MatchResult):
        ...
        self.list = sorted(self.list, key=lambda m: m.match.map + str(m.match.seed))
